...
export function useStep9Logic() {
  ...
  const handleContinue = async () => {
    ...
    const { error: e } = await upsertOnboardingData(user.id, {
      ...
      ...(returnToReview ? {} : { current_step: 9 }),
    });
    if (e) { setError('Failed to save data'); setLoading(false); return; }
    navigate(withLocalOnboardingPreview(returnToReview ? REVIEW_ROUTE : '/onboarding/step-7'));
  };
  ...
  const handleBack = () => {
    navigate(withLocalOnboardingPreview(returnToReview ? REVIEW_ROUTE : '/onboarding/step-5'));
  };
