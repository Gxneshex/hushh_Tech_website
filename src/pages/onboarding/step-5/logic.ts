  ...
  const handleSkip = async () => {
    ...
    try {
      if (config.supabaseClient) {
        const { data: { user } } = await config.supabaseClient.auth.getUser();
        if (user && !returnToReview) await upsertOnboardingData(user.id, { current_step: 7 });
      }
      navigate(withLocalOnboardingPreview(returnToReview ? REVIEW_ROUTE : '/onboarding/step-6'));
    } catch { navigate(withLocalOnboardingPreview(returnToReview ? REVIEW_ROUTE : '/onboarding/step-6')); }
