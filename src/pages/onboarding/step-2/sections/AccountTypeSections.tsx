...
/**
 * `part` lets the trust/retirement paginated wizard render one slice per sub-step
 * (details → parties → signatory). Default 'all' keeps the single-page layout used
 * by individual/joint and by review-edit mode.
 */
export type AccountTypeSectionPart = 'all' | 'details' | 'parties' | 'signatory';

export default function AccountTypeSections({
  at,
  part = 'all',
}: {
  at: UseStep3AccountTypeSections;
  part?: AccountTypeSectionPart;
}) {
  ...
  const showParties = part === 'all' || part === 'parties';
  const showSignatory = part === 'all' || part === 'signatory';

  return (
    <>
      {/* ── Retirement details (§6) ── */}
      {showDetails && accountType === 'retirement' && (
        <section className={panelClass}>
          <div className="mb-4">
            <h3 className={headingClass}>Retirement Account</h3>
            <p className={subClass}>
              Your custodian is the subscriber of record — the account is registered as
              &ldquo;[Custodian] FBO [you] IRA&rdquo;, funded from the custodian account, and the
              custodian counter-signs. Invite them below to complete their section.
            </p>
          </div>
      ...
      {showParties && cfg.requiredParties.length > 0 && <PartiesSection accountType={accountType} />}
