...
export const ACCOUNT_TYPE_CONFIG: Record<UIAccountType, AccountTypeConfig> = {
  ...
  retirement: {
    value: 'retirement',
    label: 'Retirement',
    description: 'Traditional or Roth retirement account. Custodian details may be requested.',
    ...
    requiredParties: [
      {
        ...
        label: 'Custodian',
        helpText: 'Invite the custodian/administrator if their approval or signature is required.',
        min: 0,
        max: 1,
        invitable: true,
        required: false,
    ...
    accountTypeFields: [
      ...
      { key: 'custodian_contact_email', label: 'Custodian contact email', required: false },
    ...
    requiredParties: [
      {
        role: 'trustee',
        label: 'Trustee',
        helpText: 'At least one trustee / authorised person must complete their section.',
        min: 1,
        max: 5,
        invitable: true,
        required: true,
      ...
      { role: 'authorised_person', label: 'Authorised person', min: 0, max: 1, invitable: true, required: false },
    ],
    accountTypeFields: [
      { key: 'entity_type', label: 'Entity type', required: true },
      { key: 'entity_legal_name', label: 'Legal name', required: true },
      { key: 'entity_tax_id_ein', label: 'Tax ID / EIN', required: false, sensitive: true },
      { key: 'formation_state', label: 'Formation state', required: false },
      { key: 'formation_country', label: 'Formation country', required: false },
      { key: 'registered_address', label: 'Registered address', required: false },
